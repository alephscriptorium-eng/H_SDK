...
| ficha | qué | CA | estado |
| ----- | --- | -- | ------ |
| H01 | raspa + ADR 0001/0002 + plan | mundo git limpio, espejo skills commiteado | 🔄 |
| H02 | estación calibrada + handoffs por skill | `plan/ESTACION.md` con los 8 skills | 🔄 |
| H03 | **E1** sondeo de tipos desde registry (`npm view @zeus/*` + `tsc` estricto contra `.d.ts`) | matriz paquete×types con evidencia | ⬜ |
| H04 | reporte a G: gap de `types` en `ciudad`/`startpack-ciudad` (decisión ④) | nota entregada al owner | ⬜ |
